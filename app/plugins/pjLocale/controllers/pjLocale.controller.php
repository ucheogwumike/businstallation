...
class pjLocale extends pjLocaleAppController
{
	...
	public function pjActionDeleteLocale()
	{
		...
		if ($this->isXHR())
		{
			...
			if ((int) $arr['is_default'] === 1)
			{
				pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 102, 'text' => 'Can\'t delete the default locale.'));
			}
			
			if ($pjLocaleModel->reset()->set('id', $_GET['id'])->erase()->getAffectedRows() != 1)
			{
				pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 103, 'text' => 'Locale has not been deleted.'));
			}
	...
	public function pjActionImport()
	{
		...
		if (isset($_POST['import']) && $this->isLoged() && $this->isAdmin())
		{
				...
				&& !empty($_SESSION[$_POST['key']]['separator']))
			{
				...
				{
					$locale_pairs = pjLocaleModel::factory()
						->whereIn('t1.language_iso', $_POST['locale'])
						->findAll()
						->getDataPair('language_iso', 'id');
					
					if (empty($locale_pairs))
					{
						exit;
					}
					...
					$multi_lang_arr = $pjMultiLangModel
						...
						->whereIn('t1.locale', $locale_pairs)
	...
	public function pjActionExport()
	{
		...
		if (isset($_POST['export']) && isset($_POST['separator']) && $this->isLoged() && $this->isAdmin())
		{
			...
			foreach ($locale_arr as $id => $data)
			{
				...
				$header[] = $data['language_iso'] . '::' . $title;
			}
	...
	public function pjActionSaveDefault()
	{
		...
		if ($this->isXHR())
		{
			$affected_rows = pjLocaleModel::factory()
				->where(1,1)
			...
			if ($affected_rows)
			{
				pjAppController::jsonResponse(array('status' => 'OK', 'code' => 200, 'text' => 'Default locale has been changed.'));
			}
			pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 100, 'text' => 'Default locale has not been changed.'));
		}
		exit;
	}
